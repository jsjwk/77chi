...
<script type="text/javascript">
	function deleteAll(checkName){
		...
		if(arr == ""){
			...
			if(confirm("确定要删除选中的所有消息吗？")){
				location.href = "/inbox/batchDelete/" + arr + "/" + ${page.curPage};
			}
		}
	}
	...
	function deleteOne(id){
		if(confirm("确定要删除这条消息吗？")){
			location.href = "/inbox/delete/" + id + "/" + ${page.curPage};
		}
	}
			...
			<div class="article notice">
				...
				<div class="noti_right">
					<div class="top_notiright">
						...
						<span class="noti_num">${inboxMessage.total}条信息未读，共${page.total}条</span>
					</div>
					<ul>
						<c:forEach items="${inboxList}" var="inbox">
							<c:forEach items="${userList}" var="user">
								<c:if test="${inbox.sender == user.id}">
									<li>
										<input type="checkbox" name="checkbox" value="${inbox.id}" />
										<%-- <img src="${user.tinyUrl}"/> --%>
										<div class="noti_txt">
										<p>${user.userName}</p>
										<c:if test="${inbox.isRead == 1}">
											<p class="noti_tit"><a href="/inbox/detail/${inbox.id}/${page.curPage}">${inbox.title}</a></p>
										</c:if>
										<c:if test="${inbox.isRead == 0}">
											<p class="noti_tit"><b><a href="/inbox/detail/${inbox.id}/${page.curPage}">${inbox.title}</a></b></p>
										</c:if>
										</div>
										<span><script>document.write(getSmpFormatDate(new Date(${inbox.sendTime}), true));</script></span>
									</li>
								</c:if>
							</c:forEach>
						</c:forEach>
