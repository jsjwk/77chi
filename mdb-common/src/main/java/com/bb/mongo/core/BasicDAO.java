...
public class BasicDAO<T, K> implements IDAO<T, K> {
	protected Class<T> entityClazz;
	protected DatastoreImpl ds;
	Logger logger = Logger.getLogger(getClass());

	public BasicDAO(Class<T> entityClass, String dbName) {
		try {
			Properties p = new Properties();

			p.load(BasicDAO.class.getResourceAsStream(MongoConstants.FILENAME));
			String configs = p.getProperty(dbName);
			String[] config = configs.split("#");
			ServerAddress sa = new ServerAddress(config[0]);
			MongoOptions options = setMongoOptions();

			Mongo mongo = new Mongo(sa, options);
			DB db = mongo.getDB(config[1]);
			Morphia morphia = new Morphia();
			AdvancedDatastore ds = (AdvancedDatastore) morphia.createDatastore(mongo, db.getName());
	...
	private static MongoOptions setMongoOptions() {
